...
#P-MATCH - FILE HASHER AND STRING MATCHER for Windows Systems
...
from struct import unpack #Was going to use for unpacking certain C-structs like PE header info but haven't yet
...
# allfiles #not currently used
global hash #temp hash string
#global allhashes #not currently used 
...
def prepRecur(): #Reads all files recursively from input 'path' and passes to check for getting rid of bad files, checking size and passing to hash
    #try:
...
def getPath2(): #Gets path for database/binary sample as user input, checks if valid directory/file exist before succeeding
    ...
    if (int(mode) == 1):
        ...
        if (os.path.isdir(os.path.normpath(path)) == True) and (os.path.isfile(ntpath.basename(path)) == False):
            print("ERROR : DIRECTORY EXISTS BUT FILE NOT FOUND")
            getPath2()
        elif (os.path.isdir(os.path.normpath(path)) == False) and (os.path.isfile(ntpath.basename(path)) == True):
...
def scanDB(): #Scans/Imports database signature information
    ...
    global dumpstrings
    dumpstrings = []
    os.chdir(os.path.normpath(path)) #Reverts to home folder
...
def stringCompare(fname, a, b): #Where A is list of strings dumped from temp. file and B is list of strings dumped from sample binary or signature file
    ...
    while x < lensb: #iterates through all strings from binary dump
        ...
        while y < lensa: #iterates through all strings in temp file
            ...
            if (p == q):
                ...
                #print(p) #testing
                #print(str(p)) #testing
                stringmatches.append(str(p))
                break #breaks if string detected, goes to next string from bin.dump
        x = x + 1
    ...
    print(str(percmatch)+"% string match with source binary or database, matching strings listed below.")
    print(stringmatches)
...
def finished(): #Runs once all other functions complete, writes matchlist, skiplist, prints files which had non-0 % match
    ...
    if (i == 0):
        ...
        while x < i: #Iterates to print list of non-0 string-matches as given in stringCompare
            ...
            b = matchlist[x]
            x = x + 1
            print(str(a)+" : "+str(b)+"% with following strings "+str(as1)) #prints filenames, percentage match detected
    if  (i == 0):
        pass
    else:
        try:
            ...
            if (i > 0):
                while x < i:
                    ...
                    b = matchlist[x]
                    x = x + 1
                    file.write(str(a)+" "+str(b)+"%,  "+str(as1)+" ""\n") #Comma-delimited output, can change
                file.close()
